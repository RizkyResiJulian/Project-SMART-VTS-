...
def input_identitas(identitas, id_petugas, latitude, longitude):
    ...
    try:
        ...
        cursor.execute("SELECT nama_kapal, zona, jenis_kapal FROM data_kapal WHERE MMSI = %s", (mmsi,))
        kapal_data = cursor.fetchone()

        cursor.execute("SELECT nama FROM daftar_petugas WHERE id_petugas = %s", (id_petugas,))
        petugas = cursor.fetchone()

        if kapal_data:
            nama_kapal, zona_kapal, jenis_kapal = kapal_data
            nama_petugas = petugas[0] if petugas else "Tidak diketahui"
            ...
            cursor.execute("""
                INSERT INTO data_monitor (id_monitor, id_petugas, MMSI, id_agen, latitude, longitude, pelabuhan_tujuan)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """, (new_id, id_petugas, mmsi, identitas, latitude, longitude, 'Cirebon'))
            db.commit()

            return nama_kapal, zona_kapal, jenis_kapal, new_id
        else:
...
if len(sys.argv) >= 5:
    ...
    nama_kapal_ditemukan, zona_kapal, jenis_kapal, id_monitor = input_identitas(identitas_input, id_petugas_login, latitude, longitude)
...
def extract_important_info(input_str, keyword):
    ...
    if keyword == "pemberangkatan":
        patterns = [
            r"berangkat(?: dari)?\s+([a-zA-Z\s]+)",
            r"(?:keberangkatan|asal)(?: kapal| kami)?(?: dari)?\s+([a-zA-Z\s]+)",
            r"sudah berangkat dari\s+([a-zA-Z\s]+)",
            r"asal pelabuhan\s+([a-zA-Z\s]+)",
            r"kapal(?: kami)? berasal dari\s+([a-zA-Z\s]+)"
        ]
    elif keyword == "maksud":
        patterns = [
            r"(?:tujuan|maksud|keperluan|niat)(?: kedatangan| kami)?(?: adalah| untuk)?\s+([a-zA-Z\s]+)",
            r"(?:kami|kapal)(?: datang| berlabuh)(?: ke sini| ke cirebon)?(?: untuk)?\s+([a-zA-Z\s]+)",
            r"keperluan(?: kapal| kami)?(?: adalah)?\s+([a-zA-Z\s]+)",
            r"kami(?: bermaksud| bertujuan)(?: untuk)?\s+([a-zA-Z\s]+)",
            r"(?:kami ke sini|kami ke cirebon)(?: untuk)?\s+([a-zA-Z\s]+)"
        ]
    elif keyword == "muatan":
        patterns = [
    ...
    # Cek semua pola sampai ketemu yang cocok
...
KATA_KUNCI_PENTING = [
    "emergency", "darurat", "kerusakan", "kebocoran", "bocor", "kebakaran", 
...
pemberangkatan = confirm_info("pemberangkatan", extract_important_info(get_voice_input("Tolong informasikan kept, asal keberangkatan kapal darimana? Over."), "pemberangkatan"), keyword="pemberangkatan")
update_field(id_monitor, "pelabuhan_asal", pemberangkatan)

maksud = confirm_info("maksud", extract_important_info(get_voice_input("Tolong informasinya kept untuk konfirmasi kembali, maksud kedatangan kapal ke pelabuhan cirebon untuk apa? Over."), "maksud"), keyword="maksud")
update_field(id_monitor, "maksud_kedatangan", maksud)
...
while True:
    eta_input = get_voice_input("Mohon diinformasikan estimasi waktu tiba di pelabuhan cirebon jam berapa, Kept? Misalnya 'besok pagi' atau 'jam 9 malam'. Over.")
...
kapten = confirm_info("nama kapten", extract_important_info(get_voice_input("Tolong informasikan kepada kami nama lengkap kapten? Over."), "nama kapten"), keyword="nama kapten")
...
while True:
    ...
    if any(word in followup.lower() for word in ["cukup", "tidak", "terima kasih", "selesai"]):
        speak("Baik, terima kasih atas informasi nya kept, untuk kapal "f"{nama_kapal_ditemukan} tercatat berjenis "f"{jenis_kapal} silahkan berlabuh ke zona "f"{zona_kapal}, tolong informasikan kembali kepada kami jika kapal sudah sandar kept, V.T.S Cirebon kembali standby channel 16, 12 over and out.")
        log_chat("AI", f"Baik, terima kasih atas informasi nya capt, untuk kapal "f"{nama_kapal_ditemukan} tercatat berjenis "f"{jenis_kapal} silahkan berlabuh ke zona "f"{zona_kapal}, tolong informasikan kembali kepada kami jika kapal sudah sandar kept, VTS Cirebon kembali standby channel 16, 12 over and out.")
        print("AI: Baik, terima kasih atas informasi nya capt, untuk kapal "f"{nama_kapal_ditemukan} tercatat berjenis "f"{jenis_kapal} silahkan berlabuh ke zona "f"{zona_kapal}, tolong informasikan kembali kepada kami jika kapal sudah sandar kept, VTS Cirebon kembali standby channel 16, 12 over and out.")
        break